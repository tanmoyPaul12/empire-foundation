          ...
          <tr>
            ...
            <td>
              ${
                v.status === 'pending'
                ? `
                  <button onclick="approveVolunteer('${v._id}')">
                    Approve
                  </button>
...
async function resolveSOS(id) {
  await fetch(`${API_BASE}/api/sos/${id}`, {
    method: 'PUT'
  });
  ...
  data.data.forEach(item => {
    ...
    html += `
      <tr>
        ...
        <td>
          <a href="${API_BASE}/uploads/${item.image}" target="_blank">
            Open Img
          </a>
        </td>
...
async function loadBloodDonors() {
  ...
  try {
    ...
    if (result.success && result.data.length > 0) {
      ...
      result.data.forEach(donor => {
        ...
        html += `
          <tr>
            ...
            <td>
              <span class="${donor.available ? 'status-available' : 'status-unavailable'}">
                ${donor.available ? 'Available' : 'Unavailable'}
              </span>
            </td>
      ...
      container.innerHTML = "No blood donors yet";
    }

  } catch (error) {

    container.innerHTML = "Error loading blood donors";
  }
}
